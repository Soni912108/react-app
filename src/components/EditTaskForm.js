...
const EditTaskForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`http://localhost:3001/api/tasks/getOneTask/${taskId}`, {
          headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3001/api/tasks/updateTask/${taskId}`, task, {
        headers: {
